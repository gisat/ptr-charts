...
class AxisY extends React.PureComponent {
	...
	static propTypes = {
		...
		stacked: PropTypes.oneOfType([PropTypes.string, PropTypes.bool]),
	...
	render() {
		...
		return (
			<g className="ptr-column-chart-axis-y" transform={`translate(0,0)`}>
				{props.options && props.options.highlightedArea
					? this.renderHighlightedArea()
					: null}
				{props.ticks || props.gridlines || props.withValues
					? this.renderGrid()
					: null}
	...
	renderBorder() {
		...
		const xCoordRight =
			this.props.width + this.props.labelSize + this.props.plotWidth - 1;
		return (
			<>
				<path
					className="ptr-axis-border ptr-axis-border-right"
					d={`M${xCoordRight} ${this.props.height} L${xCoordRight} 0`}
				/>
				{this.props.diverging ? (
					<path
						className="ptr-axis-border ptr-axis-border-left"
						d={`M${xCoordLeft} ${this.props.height} L${xCoordLeft} 0`}
					/>
				) : null}
			</>
		);
	...
	renderGrid() {
		const props = this.props;
		const tickCount =
			this.props.options && this.props.options.tickCount
				? this.props.options.tickCount
				: TICK_COUNT;

		let shift = props.ticks
			? TICK_SIZE + TICK_CAPTION_OFFSET_VERTICAL
			: TICK_CAPTION_OFFSET_VERTICAL;
		...
		return (
			<g
				className="ptr-axis-grid"
				transform={`translate(${props.width + props.labelSize - shift},${
					props.topPadding
				})`}
			>
				{ticks.map(value => {
